                            ...
                            GError **error)
{
	...
	if (!priv->rf_enabled) {
		nm_utils_error_set_literal (error, "RFKILL for modem enabled");
		return FALSE;
	}

	if (!priv->modem) {
		nm_utils_error_set_literal (error, "modem not available");
		return FALSE;
	}

	state = nm_modem_get_state (priv->modem);
	if (state <= NM_MODEM_STATE_INITIALIZING) {
		nm_utils_error_set_literal (error, "modem not initalized");
		return FALSE;
	}

	if (state == NM_MODEM_STATE_LOCKED) {
		if (!nm_connection_get_setting_gsm (connection)) {
			nm_utils_error_set_literal (error, "modem is locked without pin available");
			return FALSE;
		}
	}
