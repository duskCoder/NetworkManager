                            ...
                            GError **error)
{
	...
	if (!NM_IN_SET (mode, NULL, NM_SETTING_WIRELESS_MODE_INFRA)) {
		nm_utils_error_set_literal (error, "iwd only supports infrastructure mode connections");
		return FALSE;
	}

	/* Hidden SSIDs not supported yet */
	if (nm_setting_wireless_get_hidden (s_wifi)) {
		nm_utils_error_set_literal (error, "hidden networks not supported by iwd");
		return FALSE;
	}
	...
	if (get_connection_iwd_security (connection) == NM_IWD_NETWORK_SECURITY_8021X) {
		if (!is_connection_known_network (connection)) {
			nm_utils_error_set_literal (error, "network is not known to iwd");
			return FALSE;
		}
	}
	...
	if (specific_object) {
		...
		if (!ap) {
			nm_utils_error_set_literal (error, "requested access point not found");
			return FALSE;
		}
		if (!nm_wifi_ap_check_compatible (ap, connection)) {
			nm_utils_error_set_literal (error, "requested access point is not compatible with profile");
			return FALSE;
		}
		return TRUE;
	}
	...
	if (!nm_wifi_aps_find_first_compatible (&priv->aps_lst_head, connection)) {
		nm_utils_error_set_literal (error, "no compatible access point found");
		return FALSE;
	}
