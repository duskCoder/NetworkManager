...
check_connection_compatible (NMDevice *device, NMConnection *connection, GError **error)
{
	...
	if (nm_device_is_real (device)) {
		...
		if (!_same_og (nm_setting_tun_get_owner (s_tun), priv->props.owner_valid, priv->props.owner)) {
			nm_utils_error_set_literal (error, "tun owner setting mismatches");
			return FALSE;
		}
		if (!_same_og (nm_setting_tun_get_group (s_tun), priv->props.group_valid, priv->props.group)) {
			nm_utils_error_set_literal (error, "tun group setting mismatches");
			return FALSE;
		}
		if (nm_setting_tun_get_pi (s_tun) != priv->props.pi) {
			nm_utils_error_set_literal (error, "tun pi setting mismatches");
			return FALSE;
		}
		if (nm_setting_tun_get_vnet_hdr (s_tun) != priv->props.vnet_hdr) {
			nm_utils_error_set_literal (error, "tun vnet-hdr setting mismatches");
			return FALSE;
		}
		if (nm_setting_tun_get_multi_queue (s_tun) != priv->props.multi_queue) {
			nm_utils_error_set_literal (error, "tun multi-queue setting mismatches");
			return FALSE;
		}
	}
