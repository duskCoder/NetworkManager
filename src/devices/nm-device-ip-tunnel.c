...
check_connection_compatible (NMDevice *device, NMConnection *connection, GError **error)
{
	...
	if (nm_device_is_real (device)) {
		...
		if (parent && !nm_device_match_parent (device, parent)) {
			nm_utils_error_set_literal (error, "IP tunnel parent mismatches");
			return FALSE;
		}
		...
		if (!address_equal_pp (priv->addr_family,
		                       nm_setting_ip_tunnel_get_local (s_ip_tunnel),
		                       priv->local)) {
			nm_utils_error_set_literal (error, "local IP tunnel address mismatches");
			return FALSE;
		}
		...
		if (!address_equal_pp (priv->addr_family,
		                       nm_setting_ip_tunnel_get_remote (s_ip_tunnel),
		                       priv->remote)) {
			nm_utils_error_set_literal (error, "remote IP tunnel address mismatches");
			return FALSE;
		}

		if (nm_setting_ip_tunnel_get_ttl (s_ip_tunnel) != priv->ttl) {
			nm_utils_error_set_literal (error, "TTL of IP tunnel mismatches");
			return FALSE;
		}

		if (nm_setting_ip_tunnel_get_tos (s_ip_tunnel) != priv->tos) {
			nm_utils_error_set_literal (error, "TOS of IP tunnel mismatches");
			return FALSE;
		}

		if (priv->addr_family == AF_INET) {
			if (nm_setting_ip_tunnel_get_path_mtu_discovery (s_ip_tunnel) != priv->path_mtu_discovery) {
				nm_utils_error_set_literal (error, "MTU discovery setting of IP tunnel mismatches");
				return FALSE;
			}
		} else {
			if (nm_setting_ip_tunnel_get_encapsulation_limit (s_ip_tunnel) != priv->encap_limit) {
				nm_utils_error_set_literal (error, "encapsulation limit of IP tunnel mismatches");
				return FALSE;
			}

			if (nm_setting_ip_tunnel_get_flow_label (s_ip_tunnel) != priv->flow_label) {
				nm_utils_error_set_literal (error, "flow-label of IP tunnel mismatches");
				return FALSE;
			}
		}
	}
