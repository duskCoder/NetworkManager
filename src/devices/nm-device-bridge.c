                            ...
                            GError **error)
{
	...
	if (s_bt) {
		...
		if (!nm_bt_vtable_network_server) {
			nm_utils_error_set_literal (error, "bluetooth plugin not available to activate NAP profile");
			return FALSE;
		}
		...
		if (!nm_bt_vtable_network_server->is_available (nm_bt_vtable_network_server, bdaddr)) {
			if (bdaddr)
				nm_utils_error_set (error, "not suitable NAP device \"%s\" available", bdaddr);
			else
				nm_utils_error_set_literal (error, "not suitable NAP device available");
			return FALSE;
		}
	}
